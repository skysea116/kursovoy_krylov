...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Фев 06 2023 г., 01:02
...
(2, 2, 'Алюминий AL-6061', 'Easing ES-225', '15.90 кг', 'Microshift TS-38', 'DNP LY-G818', 'Shimano Tourney RD-TY21', 'GS-S307, сталь', 'Neco B-910, картридж', 'DNP TM-1007, трещотка', 21, 'Taya TB-50', 'MX-P624, пластик', 27.5, 'Rainbow DH-18', '-', 'Anteng, конусные', 'Citron CT-1201 2,1\"', 'Sparkle VB-N968SK', 'Sparkle VB-N968SK', 'Сталь, 25,4 мм, 620 мм', 'Сталь, резьбовой, 25,4 мм, 100 мм', 'Неинтегрированная', 'YBT YBT-1017', '27,2 мм, 350 мм', 'Россия', 'КНР (Тайвань)'),
...
(5, 5, 'Сталь HI-TENSILE', 'Жёсткая', '10.10 кг', '-', '-', '-', '-', 'Сталь', '-', 1, '76 link', 'Пластик', 26, 'Алюминиевые', '-', 'Сталь, Гайка', '18\"×2.125', '-', 'DACHANG', 'Novatrack, Сталь', 'Novatrack, Резьбовой 1\", Сталь', 'Резьбовая 1\"', 'Novatrack', '25,4мм; длина=200 мм, Сталь', '-', '-'),
...
INSERT INTO `delivery_addresses` (`address_id`, `user_id`, `city`, `street`, `house`, `corpus`, `building`, `entrance`, `floor`, `apartment`) VALUES
(21, 9, 'Москва', 'Тайнинская', '13', '1', '', '1', 7, '147');
...
(1, 'Велосипед STINGER GRAPHITE EVO 29', 38990, 'Stinger Graphite Evo – относится к категории горных велосипедов, оснащенных колесами диаметром 29 дюймов. Отлично подойдёт для ценителей быстрой езды по городским асфальтовым дорогам и пересечённой местности. Облегченная рама выполнена из уникального алюминиевого сплава. Велобайк оборудован надежной гидравлической тормозной системой, обеспечивающей быстрое и плавное торможение. Оборудование известного бренда Shimano делают переключение скоростных режимов точным и быстрым.', 1, 'good1.jpeg', 6),
(2, 'Велосипед Stark Outpost 26.1 V (2022)', 17990, 'Велосипед Stark Outpost 26.1 V представляет собой яркий и стильный хардтейл. Это идеальное решение, сочетающее доступную цену и отличное качество. Модель одинаково хорошо демонстрирует свои эксплуатационные показатели как при езде по пересеченной местности, так и в условиях городских улиц.\r\nИз основных преимуществ байка Outpost 26.1 V выделяют:\r\n•    особопрочная рама из облегченного алюминия;\r\n•    21 скорость, благодаря которой вы легко подбираете оптимальный для себя вариант езды;\r\n•    надежная система тормозов вида V-brake, гарантирующая эффективное торможение;\r\n•    улучшенная вилка амортизации с ходом в 60 мм;\r\n•    26 типоразмеров колес, отличающихся повышенной жесткостью.\r\nПроизводитель использует в конструкции оригинальную вилку Easing ES-225, что обеспечивает байк отличными амортизационными показателями и позволяет сохранять максимально точную управляемость  даже при заездах по труднодоступным участкам. В комплекте идет удобная подножка и крылья.', 1, 'good2.jpeg', 7),
...
INSERT INTO `goods_in_orders` (`order_goods_id`, `order_id`, `good_id`, `quant`) VALUES
(19, 28, 1, 2),
(20, 28, 2, 1),
(21, 28, 3, 1),
(22, 28, 5, 1);
...
INSERT INTO `orders` (`order_id`, `user_id`, `goods_count`, `order_price`, `status_id`, `order_date`, `address_id`) VALUES
(28, 9, 5, 122670, 1, '2023-02-06 00:57:13', 21);
...
INSERT INTO `users` (`user_id`, `user_name`, `user_surename`, `user_login`, `user_email`, `role_id`, `password`, `user_phone`) VALUES
(9, 'Александр', 'Крылов!!!', 'sanya', 'alex@krylov.ru', 3, 'e3075e260af061e77c511047dade9f03', '88003455667'),
(10, 'admin', 'admin', 'admin', 'admin@admin.ru', 1, '95a7f72719a14f49cc9883bfc13a212f', '123456');
...
--
ALTER TABLE `delivery_addresses`
  MODIFY `address_id` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=22;

--
-- AUTO_INCREMENT для таблицы `goods`
--
ALTER TABLE `goods`
  MODIFY `good_id` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;

--
-- AUTO_INCREMENT для таблицы `goods_in_orders`
--
ALTER TABLE `goods_in_orders`
  MODIFY `order_goods_id` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=23;
...
--
ALTER TABLE `goods_in_orders`
  ADD CONSTRAINT `goods_in_orders_ibfk_1` FOREIGN KEY (`order_id`) REFERENCES `orders` (`order_id`),
  ADD CONSTRAINT `goods_in_orders_ibfk_2` FOREIGN KEY (`good_id`) REFERENCES `goods` (`good_id`);
...
ALTER TABLE `orders`
  ...
  ADD CONSTRAINT `orders_ibfk_4` FOREIGN KEY (`address_id`) REFERENCES `delivery_addresses` (`address_id`);
